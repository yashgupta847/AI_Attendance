...
# from src.screens.teacher_screen import teacher_dashboard
...
from src.components.dialog_share_card import share_subject_dialog
...
def register_teacher(teacher_username , teacher_name , teacher_pass , teacher_confirm_pass):
    ...
    if check_teacher_exist(teacher_username):
        return False , "This Username Not available"
    if teacher_pass != teacher_confirm_pass:
        return False , "Password Doesn't match"

    try:
        create_teacher(teacher_username , teacher_pass , teacher_name)
        return True , "Teacher Created his/her profile successfully"
...
def teacher_tab_take_attendance():
    ...
    if not subjects:
        ...
        return
    subject_options = {  f'{s["name"]} - {s["subject_code"]}': s["subject_id"] for s in subjects} #type:ignore
...
def teacher_tab_manage_subjects():
    ...
    if subjects:
        for sub in subjects:
            stats = [
                ("🫂", "Students", sub['total_student']),  #type:ignore
                ("🕰️", "Classes", sub['total_classes']), #type:ignore
            ]
            def share_btn():
                if st.button(f"Share Code: {sub['name']}", key=f"share_{sub['subject_code']}", icon=":material/share:"):  #type:ignore
                    share_subject_dialog(sub['name'], sub['subject_code']) #type:ignore
                    st.space()
            subject_card(
...
def teacher_screen_login():
    ...
    st.space()
    teacher_username = st.text_input("Enter username" , placeholder="ananyroy")
    ...
    with btnc1:
        if st.button("Login" , icon=":material/passkey:" , shortcut="control+enter" , width="stretch"):
            if login_teacher(teacher_username , teacher_pass):
                ...
                st.error("Invalid username or password ! Try Again Later with valid credentials")
    with btnc2:
        if st.button("Register Instead" , icon=":material/passkey:" , shortcut="control+enter" , width="stretch"):
...
def teacher_screen_register():
    ...
    st.header("Regiter your Teacher Profile")
    st.space()
    teacher_username = st.text_input("Enter username" , placeholder="ananyroy")
    teacher_name = st.text_input("Enter name" , placeholder="Abhishek Sharma")

    teacher_pass = st.text_input("Enter password" , type="password" , placeholder="........")
    teacher_confirm_pass = st.text_input("Confirm Your password" , type="password" , placeholder="........")
    st.divider()
    btnc1 , btnc2 = st.columns(2)
    with btnc1:
